...
export default function Skills() {
	return (
		<article className="flex bg-neutral-100 rounded-3xl overflow-hidden max-h-[18rem] min-h-[18rem] relative flex-col w-full shadow-md p-8">
			...
			<header className="flex flex-col z-5">
				...
				<p className="text-black/85 text-[1.05rem] leading-6 font-geist max-w-[85%] font-medium">Technologies I use and skills I've learned</p>
			</header>
				...
				<div className="flex w-max min-w-full shrink-0 flex-nowrap gap-8 py-3 animate-scrolling hover:[animation-play-state:paused] h-full">
					{[...SKILLS, ...SKILLS].map((skill, i) => (
						<div key={i} className="group flex flex-col items-center justify-center gap-4 min-w-[100px] cursor-pointer">
							<skill.icon className="w-12 h-12 text-black/75 transition-all group-hover:text-black group-hover:scale-125" />
							<span className="group-hover:text-black text-sm text-black/75">{skill.name}</span>
						</div>
					))}
				</div>
			</footer>
