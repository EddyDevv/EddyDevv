	...
	return (
		<div className="w-full flex gap-2 flex-wrap max-[850px]:flex-nowrap max-[850px]:flex-col">
			{projects.map(project => (
				<article key={project.name} className="flex bg-neutral-100 rounded-3xl overflow-hidden flex-1 max-h-[27.5rem] min-h-[27.5rem] min-w-[16rem] relative flex-col p-4 shadow-md">
					<header className="flex justify-between w-full z-5 p-4">
						<div className="flex flex-col">
							<h1 className="font-semibold font-rubik text-2xl">{project.name}</h1>
							<p className="text-black/85 text-[1rem] leading-5 font-geist font-medium">{project.description}</p>
						</div>
						<div className="flex flex-col items-center justify-start">
							<Link
								...
								target="_blank"
								rel="noreferrer"
								className="items-center justify-center flex bg-black size-[2.5rem] rounded-full transition-all ease-out duration-300 hover:scale-105 hover:translate-x-2 hover:-translate-y-2 active:scale-95 active:translate-x-1 active:-translate-y-1 active:opacity-90"
							>
								<NewWindowIcon className="size-5 text-white" />
							</Link>
						</div>
					</header>
					<footer className="w-full flex-wrap gap-2 flex mt-auto z-5 relative overflow-hidden rounded-2xl h-[15rem]">
						<div className="absolute inset-0 z-1 bg-black/10 animate-pulse"></div>
